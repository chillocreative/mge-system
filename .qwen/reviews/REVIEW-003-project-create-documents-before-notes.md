...
1. `fix(project-create)` — `resources/js/pages/projects/ProjectCreate.jsx`, `public/build/manifest.json`, `public/build/assets/app-BQTbRQ0m.js` (+) and `app-D2mMpR_t.js` (−).
2. `docs(orchestrator)` — SPEC-003, this review, `.qwen/specs/README.md` numbering note. Code and records kept apart so the production tree only ever receives the first.
