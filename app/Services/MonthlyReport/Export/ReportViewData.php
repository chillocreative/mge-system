...
/**
 * Builds the shared view-data payload (report, ctx, sections, notes, logos, titles, …) used by
 * both the PDF exporter (via view rendering) and the DOCX exporter. Extracted from
 * PdfExporter::viewData() so both exporters build from a single, tested source.
 */
final class ReportViewData
{
    ...
    /**
     * @return array{report: MonthlyReport, ctx: ReportContext, sections: array, notes: array,
     *               logos: array, mgeLogo: ?string, titles: array, tocKeys: array<int, string>}
     */
    public static function build(MonthlyReport $report): array
    {
        return (new self)->viewData($report);
    }
    ...
    private function viewData(MonthlyReport $report): array
    {
        ...
        foreach (['2.2', '2.4'] as $key) {
            ...
            // A malformed override must degrade to "no chart", never abort the whole export.
            try {
        ...
        $tocKeys = array_values(array_intersect(array_keys(SectionRegistry::TITLES), array_keys($sections)));
